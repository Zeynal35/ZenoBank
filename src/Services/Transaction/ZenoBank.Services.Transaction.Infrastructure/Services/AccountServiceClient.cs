...
public class AccountServiceClient : IAccountServiceClient
{
    ...
    public async Task<Result<InternalAccountSnapshotDto>> GetAccountByIdAsync(Guid accountId, CancellationToken cancellationToken = default)
    {
        ...
        var content = await response.Content.ReadFromJsonAsync<ServiceApiResponse<InternalAccountSnapshotDto>>(cancellationToken: cancellationToken);

        if (!response.IsSuccessStatusCode || content is null || !content.Success || content.Data is null)
            return Result<InternalAccountSnapshotDto>.Failure(content?.Message ?? "Failed to fetch account.", content?.Errors);
    ...
    {
        var response = await _httpClient.PostAsJsonAsync("internal/accounts/increase-balance",
            new { AccountId = accountId, Amount = amount }, cancellationToken);

        var content = await response.Content.ReadFromJsonAsync<ServiceApiResponse<AccountBalanceSnapshotDto>>(cancellationToken: cancellationToken);

        if (!response.IsSuccessStatusCode || content is null || !content.Success || content.Data is null)
            return Result<AccountBalanceSnapshotDto>.Failure(content?.Message ?? "Failed to increase balance.", content?.Errors);
    ...
    {
        var response = await _httpClient.PostAsJsonAsync("internal/accounts/decrease-balance",
            new { AccountId = accountId, Amount = amount }, cancellationToken);

        var content = await response.Content.ReadFromJsonAsync<ServiceApiResponse<AccountBalanceSnapshotDto>>(cancellationToken: cancellationToken);

        if (!response.IsSuccessStatusCode || content is null || !content.Success || content.Data is null)
            return Result<AccountBalanceSnapshotDto>.Failure(content?.Message ?? "Failed to decrease balance.", content?.Errors);
    ...
    {
        var response = await _httpClient.PostAsJsonAsync("internal/accounts/transfer-balance",
            new { FromAccountId = fromAccountId, ToAccountId = toAccountId, Amount = amount }, cancellationToken);

        var content = await response.Content.ReadFromJsonAsync<ServiceApiResponse<object>>(cancellationToken: cancellationToken);

        if (!response.IsSuccessStatusCode || content is null || !content.Success)
            return Result.Failure(content?.Message ?? "Failed to transfer balance.", content?.Errors);
